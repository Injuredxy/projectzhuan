...
Page({
    ...
    wx.getImageInfo({
      ...
      success(res) {
        ...
        if (price < 1000) {
          ...
          } else if (old_price < 1000000) {
            cx.lineTo(630 * pixe / 2, 514 * pixe / 2)
          }
        }
        cx.stroke()
  ...
  goAssemble() {
    ...
    if (app.globalData.but) {
      wx.navigateTo({
        url: `/pages/goSample/sample?activity_id=${activity_id}`,
      })
    }
    app.but()
  },
  ...
  onShareAppMessage: function() {
    ...
    if (that.data.options.vestStatus == '1') {
      ...
      return {
        title: that.data.treasure.share_template ? that.data.treasure.share_template.main_title : `品牌清样！${that.data.treasure.store.brand}${that.data.treasure.goods.category_name}样品特价只需${that.data.price - 0}元！`,
        imageUrl: that.data.shareImage,
        // path: `/pages/details/sample?activity_id=${that.data.treasure.id}&share_num=${'sample'}`
        path: `/pages/details/sample?activity_id=${that.data.treasure.id}&invitation_type=${options.invitation_type}&parent_id=${options.parent_id}&register_scene=${options.register_scene}&share_number=${options.share_number}`
      }
    }else {
      return {
        title: that.data.treasure.share_template ? that.data.treasure.share_template.main_title : `品牌清样！${that.data.treasure.store.brand}${that.data.treasure.goods.category_name}样品特价只需${that.data.price - 0}元！`,
        imageUrl: that.data.shareImage,
        // path: `/pages/details/sample?activity_id=${that.data.treasure.id}&share_num=${'sample'}`
        path: `/pages/index/index?activity_id=${that.data.treasure.id}&share_num=${'sample'}`
      }
    }
  }
})